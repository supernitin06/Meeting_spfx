...
console.log("meetings data : ", meetings);
  ...
  const filteredMeetings = useMemo(() => {
    ...
    return result;
  }, [meetings, searchTerm, searchType, searchFields, filters, sortKey, sortDirection]);
  ...
  const renderCalendarView = () => {
    ...
    const renderMonthGrid = () => {
      ...
      while (day <= endDate) {
        for (let i = 0; i < 7; i++) {
          const cloneDay = day;
          const dayMeetings = meetings.filter(m => isSameDay(new Date(m.startDateTime), cloneDay));
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8 border-b border-[var(--BorderGrey)] pb-3">
              <div className="flex items-center gap-8">
                <button 
                  ...
                  className={clsx("text-xl font-normal transition-colors relative p-0", activeTab === 'Today' ? "text-[#2F5596]" : "text-[var(--DisabledGrey)] hover:text-[var(--TextBlack)]")}
                >
                  Today's meetings ({todaysMeetings.length})
                  {activeTab === 'Today' && <span className="absolute -bottom-[12px] left-0 right-0 h-1 bg-[#2F5596] rounded-t-full" />}
                </button>
                <button 
                  onClick={() => setActiveTab('Upcoming')}
                  className={clsx("text-xl font-normal transition-colors relative p-0", activeTab === 'Upcoming' ? "text-[#2F5596]" : "text-[var(--DisabledGrey)] hover:text-[var(--TextBlack)]")}
                >
                  Upcoming meetings
                  {activeTab === 'Upcoming' && <span className="absolute -bottom-[12px] left-0 right-0 h-1 bg-[#2F5596] rounded-t-full" />}
                </button>
                <button 
                  onClick={() => setActiveTab('Past')}
                  className={clsx("text-xl font-normal transition-colors relative p-0", activeTab === 'Past' ? "text-[#2F5596]" : "text-[var(--DisabledGrey)] hover:text-[var(--TextBlack)]")}
                >
                  Past meetings
                  {activeTab === 'Past' && <span className="absolute -bottom-[12px] left-0 right-0 h-1 bg-[#2F5596] rounded-t-full" />}
                </button>
              </div>
